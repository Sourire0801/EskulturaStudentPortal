...
export const AdminLayout: React.FC = () => {
  ...
  // Collapsible sidebar state (persists or default expanded)
  ...
  const navItems = [
    { label: 'Dashboard', path: '/admin/dashboard', icon: <LayoutDashboard className="w-5 h-5 fill-current" /> },
    { label: 'Members Directory', path: '/admin/members', icon: <Users className="w-5 h-5 fill-current" /> },
    { label: 'Academic Programs', path: '/admin/programs', icon: <BookOpen className="w-5 h-5 fill-current" /> },
    { label: 'ESKULTURA Units', path: '/admin/eskultura-units', icon: <Layers className="w-5 h-5 fill-current" /> },
  ];

  const adminEmail = user?.email || 'admin@eskultura.edu.ph';
  const adminDisplayName = adminEmail.split('@')[0];
  ...
  return (
    <div className="min-h-screen bg-[#FFF7EB] text-[#2B2625] flex flex-col selection:bg-[#A56F63] selection:text-white">
      ...
      >
        ...
        {!isCollapsed && (
          <div className="p-4 m-3 rounded-2xl bg-[#FFF7EB] border border-[#A8A492]/20 text-left">
            <div className="text-[11px] font-bold text-[#2B2625] truncate">{adminEmail}</div>
      ...
      {/* Main App Container (Body sticks with sidebar width) */}
      <div
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-black text-[#2B2625] tracking-tight font-['Outfit']">
                ESKULTURA Administrator Portal
              </h2>
              <p className="text-xs text-[#A8A492] font-medium hidden sm:block">
                Membership Data Entry & Student Records Management
              </p>
            </div>
          </div>
